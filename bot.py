...
async def render_seat_message(message, sid, page=1, edit_message=None):
    clean_holds()
    c=db()
    ...
    try:
        if edit_message:
            ...
            await message.answer(text=text,reply_markup=K(rows),parse_mode=None)
    except Exception:
...
async def show_detail(q: CallbackQuery):
    try:
        ok=await render_seat_message(q.message,int(q.data.split(":")[1]),1)
...
async def seat_page(q: CallbackQuery):
    try:
        ...
        ok=await render_seat_message(q.message,int(sid),int(page),edit_message=q.message)
